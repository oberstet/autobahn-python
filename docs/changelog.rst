...
* Raise the ``cryptography`` floor to ``>=50`` so PyPy (including PyPy-aarch64) installs a prebuilt wheel instead of source-compiling Rust under QEMU emulation, and refresh ``pip``/``packaging`` in ``install-build-tools`` before installing so newer platform wheel tags (e.g. ``manylinux_2_34`` PyPy-aarch64) resolve to wheels. ``>=50`` is also the first release clear of the known 2026 ``cryptography`` advisories — CVE-2026-26007 (fixed 46.0.5), CVE-2026-39892 (fixed 46.0.7) and the PKCS#7 Bleichenbacher oracle (fixed 50.0.0); since ``cryptography`` is Rust-only with no pure-Python fallback it cannot be capped, so the floor is raised to the security-clean version (#1947)
